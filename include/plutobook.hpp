...
/**
 * @brief Identifies what kind of layout entity an Annotation describes.
 */
enum class AnnotationKind : uint8_t {
    Block,   ///< A box-level entity (a labelled element, or an auto-labelled block).
    Inline,  ///< A labelled inline element, or one fragment of one.
    TextLine ///< A single shaped run of text on one line.
};

/**
 * @brief A labelled rectangle on a single page, suitable for object-detection datasets.
 *
 * Coordinates are in page space, in points, with the origin at the top-left of the
 * page including its margins. They are already clipped to the visible page area.
 */
struct Annotation {
    AnnotationKind kind{AnnotationKind::Block};
    uint32_t pageIndex{0};
    float x{0};
    float y{0};
    float width{0};
    float height{0};
    std::string label;     ///< The value of the label attribute, or the auto-derived label.
    std::string id;        ///< The value of the id attribute, if any.
    std::string parentId;  ///< The id of the nearest labelled ancestor, if any.
    std::string tagName;   ///< The tag name of the originating element, if any.
    std::string text;      ///< The text content, for AnnotationKind::TextLine.
};

using AnnotationList = std::vector<Annotation>;

/**
 * @brief Controls how annotations are extracted from a laid-out document.
 */
struct AnnotationOptions {
    /** @brief The attribute carrying the annotation label. */
    std::string labelAttribute{"data-anno"};

    /** @brief The attribute carrying a stable identity for the annotated element. */
    std::string idAttribute{"data-anno-id"};

    /** @brief Emit one annotation per shaped text run. */
    bool includeTextLines{true};

    /** @brief Emit blocks that carry no label attribute, labelled by tag name. */
    bool includeUnlabelledBlocks{false};

    /**
     * @brief Emit the page margin boxes (running headers, footers, page numbers).
     *
     * Their content is generated by CSS rather than by an element, so they are
     * labelled by position: "header" along the top edge, "footer" along the bottom,
     * and "margin" along the sides.
     */
    bool includePageMargins{true};

    /** @brief Emit the union of a labelled inline element's line fragments rather than one per line. */
    bool mergeInlineFragments{false};

    /** @brief Discard annotations narrower or shorter than this, in points. */
...
public:
    ...
    /**
     * @brief Returns the labelled bounding boxes on the specified page.
     *
     * The document is laid out if it has not been already. Annotations are returned in
     * paint order, in page-space points, clipped to the visible area of the page.
     *
     * @param pageIndex The index of the page.
     * @param options Controls which entities are emitted and which attributes are read.
     * @return The annotations found on that page.
     */
    AnnotationList annotationsAt(uint32_t pageIndex, const AnnotationOptions& options = AnnotationOptions()) const;
